...
async function loadDashboard() {
  ...
  renderDayBeforeYesterdaysSessions(sessionHistory);
}
  ...
  todaySessions.forEach(session => {
    ...
    try {
      if (session.isMultiPlatform && session.platforms && session.platforms.length > 1) {
        ...
        const instagramIcon = `<svg class="platformIcon" viewBox="0 0 24 24" fill="currentColor">
...
function renderDayBeforeYesterdaysSessions(sessionHistory) {
  const container = document.getElementById('dayBeforeYesterdaySessions');
  if (!container) return;

  // Filter sessions for 2 days ago
  const today = new Date();
  const dayBeforeYesterday = new Date(today);
  dayBeforeYesterday.setDate(dayBeforeYesterday.getDate() - 2);
  const dayBeforeYesterdayKey = isoDate(dayBeforeYesterday);

  // Update header with actual date
  const headerEl = document.getElementById('dayBeforeYesterdayHeader');
  if (headerEl) {
    const options = { weekday: 'long', month: 'short', day: 'numeric' };
    headerEl.textContent = dayBeforeYesterday.toLocaleDateString('en-US', options) + ' Sessions';
  }

  const sessions = sessionHistory.filter(session => {
    const sessionDate = isoDate(new Date(session.endedAt));
    return sessionDate === dayBeforeYesterdayKey;
  });

  if (sessions.length === 0) {
    container.innerHTML = '<p style="color: var(--muted); font-size: 13px; padding: 8px 0;">No sessions recorded.</p>';
    return;
  }
